...
public class AccountManagerImplVolumeDeleteEventTest extends AccountManagetImplTestBase {
	...
	protected UsageEventUtils setupUsageUtils() throws NoSuchMethodException, SecurityException, IllegalAccessException,
		...
		List<UsageEventVO> persistedItems = new ArrayList<>();
		Mockito.when(_usageEventDao.listAll()).thenAnswer(invo -> {
			return persistedItems;
		});
		Mockito.when(_usageEventDao.persist(Mockito.any())).thenAnswer(invo -> {
			UsageEventVO entity = invo.getArgumentAt(0, UsageEventVO.class);
